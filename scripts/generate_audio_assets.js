...
function resolveFfmpeg() {
  ...
  try {
    ...
    const candidate = typeof staticPath === 'string' ? staticPath : staticPath?.path;
    if (candidate && fs.existsSync(candidate)) return candidate;
  } catch {}
  // Production builds should not fail just because the optional asset
  // synthesizer cannot find a binary. The generated MP3s are committed
  // release assets; preserve them and let the app build from those files.
  return null;
}
...
function writeWavFile(filename, sampleRate, samples) {
  ...
  if (!ffmpegPath) {
    console.warn(`Skipping ${filename}: ffmpeg binary unavailable; keeping existing release asset if present.`);
    return;
  }
  const numChannels = 1;
