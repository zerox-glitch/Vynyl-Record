...
- **Cloudflare R2:** Set `R2_UPLOAD_SECRET`, `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, and `R2_BUCKET` to activate direct browser-to-R2 voice and admin asset uploads plus processed-object storage. `R2_PUBLIC_BASE` is optional; private records and reusable melodies receive signed download URLs through stable app routes. Production admin uploads deliberately fail instead of pretending an ephemeral Vercel `/tmp` file was saved when R2 is absent.
